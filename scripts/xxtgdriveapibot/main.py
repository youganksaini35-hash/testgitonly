...
async def media_upload_handler(event):
    ...
    status_msg = await event.respond(
        f"⚡ <b>Storing in TG Drive...</b>\n\n"
        f"{icon} <code>{clean_html(file_name)}</code> ({format_bytes(file_size)})\n"
        f"📂 Target: {folder_tag}\n\n"
        f"<code>[███████████████ 100%]</code> 🚀 <i>Direct MTProto Linking (0.2s)...</i>",
        parse_mode="html"
    )
    ...
    try:
        ...
        # 🚀 ZERO-DOWNLOAD DIRECT MTPROTO COPY (Instant 0.2s - 0 MB VPS Data & Disk Used!)
        try:
        ...
        text = (
            ...
            f"⚡ <b>Speed:</b> <code>Instant (0.2s) - Zero VPS Data</code>\n"
