...
async def start_handler(event):
    ...
    if not tg_id or not tg_hash:
        ...
        text = (
            f"👋 <b>Namaste {clean_html(first_name)}! Welcome to TG Drive Cloud Manager.</b>\n\n"
            f"Bot ko use karne ke liye pehle apna Telegram API configure karein (sirf 1 baar karna hai):\n\n"
            f"1️⃣ <b>Now send me your Telegram API ID:</b>\n"
            f"<i>(Example: <code>12345678</code>)</i>\n\n"
            f"💡 <i>Agar API ID nahi hai, toh <a href=\"https://my.telegram.org\">my.telegram.org</a> par login karke 1 minute me generate kar sakte hain.</i>"
        )
    ...
    if not api_key:
        ...
        text = (
            f"👋 <b>Namaste {clean_html(first_name)}! Welcome back.</b>\n\n"
            f"Telegram API credentials verified hain! 🔒\n\n"
            f"3️⃣ <b>Now send your TG Drive API key:</b>\n"
            f"<i>(Example: <code>tgd_live_...</code>)</i>\n\n"
            f"👇 <i>Niche diye gaye button se apni personal API Key copy karke yahan paste karein:</i>"
        )
...
async def search_cmd_handler(event):
    ...
    api_key = get_user_api_key(user_id)
    if not api_key:
        await event.respond("❌ Kripya pehle /start bhej kar setup complete karein.")
        return
...
async def media_upload_handler(event):
    ...
    if not tg_id or not tg_hash or not api_key:
        await event.respond(
            ...
            "File upload karne se pehle apna account configure karein.\n\n"
            "👉 Kripya <b>/start</b> send karein aur setup complete karein.",
            parse_mode="html"
        )
        return
...
async def text_handler(event):
    ...
    if is_admin(user_id) and state == "AWAITING_BROADCAST_MSG":
        ...
        users = get_all_users()
        if not users:
            await event.respond("❌ Database me koi users nahi hain.", buttons=admin_panel_kb())
            return
    ...
    if state == "AWAITING_TG_API_ID":
        if text.isdigit() and len(text) >= 5:
            ...
            await event.respond(
                "❌ <b>Invalid API ID!</b>\n\n"
                "Telegram API ID ek number hota hai (e.g. <code>12345678</code>).\n"
                "Kripya sahi API ID send karein:",
                parse_mode="html"
            )
            return
    ...
    if state == "AWAITING_TG_API_HASH":
        if len(text) >= 20:
            ...
            await event.respond(
                ...
                "Telegram API Hash 32-character ka hex code hota hai (e.g. <code>867fafeeabc20a75163ef2ddbd877f70</code>).\n"
                "Kripya sahi API Hash send karein:",
                parse_mode="html"
            )
            return
...
async def callback_handler(event):
    ...
    if data == "menu_logout_confirm":
        ...
        await event.edit("⚠️ <b>Kya aap apna account disconnect karna chahte hain?</b>\n\nLogout karne se aapka encrypted data aur session database se delete ho jayega.", buttons=buttons, parse_mode="html")
        return
    ...
    if data == "menu_logout_do":
        ...
        file_cache.invalidate(user_id)
        await event.edit("✅ <b>Aapka account permanently logout ho gaya hai!</b>\n\nBot dobara use karne ke liye /start send karein.", buttons=api_key_request_kb(), parse_mode="html")
        return
    ...
    if data == "admin_panel":
        await event.answer()
        if not is_admin(user_id):
            await event.edit("⛔ <b>Unauthorized!</b> Sirf admins is menu ko access kar sakte hain.", buttons=back_to_main_kb(), parse_mode="html")
            return
        stats = get_bot_stats()
        text = (
            ...
            f"<i>Niche diye gaye buttons se management perform karein:</i>"
        )
    ...
    if data == "admin_broadcast_prompt":
        ...
        text = (
            ...
            "Aap jo bhi message sabhi bot users ko bhejna chahte hain, wo yahan send karein:\n"
            "• Plain text message (HTML supported)\n"
            "• Photo / Video / Media with caption\n"
            "• Document / APK / Audio\n"
            "• Forwarded post\n\n"
            "👇 <i>Kripya broadcast content yahan send karein:</i>"
        )
    ...
    if data == "admin_user_list":
        ...
        users_block = "\n".join(user_lines) if user_lines else "<i>Koi user registered nahi hai.</i>"
        text = (
    ...
    if data == "menu_upload_guide":
        ...
        text = (
            ...
            "2️⃣ Zero-Download Direct MTProto Engine 0.3s me use TG Drive Cloud par store kar dega!"
        )
        await event.edit(text, buttons=back_to_main_kb(), parse_mode="html")
        return
