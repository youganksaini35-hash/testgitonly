            ...
            if os.path.exists(CLOUDFLARE_BIN):
                ...
                if discovered_url:
                    ...
                    while proc.poll() is None:
                        ...
                        # 3. Active probe: ensure tunnel is reachable
                        try:
                            import httpx
                            async with httpx.AsyncClient(timeout=5.0) as probe_client:
                                ...
                                if probe_res.status_code != 200:
                                    print(f"⚠️ [Watchdog] Tunnel returned status {probe_res.status_code}. Restarting tunnel...")
                                    try: proc.terminate()
                                    except Exception: pass
                                    break
                        except Exception as probe_err:
                            print(f"⚠️ [Watchdog] Tunnel probe failed ({probe_err}). Reconnecting...")
                            try: proc.terminate()
                            except Exception: pass
                            break

                print("⚠️ [Tunnel Supervisor] Tunnel process exited. Re-launching in 3 seconds...")
