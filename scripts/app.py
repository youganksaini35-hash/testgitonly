...
Powered by FastAPI & Telethon MTProto (Auto Direct Tunnel & Session Normalizer)
"""
...
def _start_direct_tunnel():
    """Directly runs cloudflared to print public HTTPS link in logs"""
    try:
        cf_bin = "/tmp/cloudflared"
        if not os.path.exists(cf_bin):
            print("[Tunnel] Downloading standalone Cloudflare binary...", flush=True)
            ...
            print("[Tunnel] Binary installed successfully!", flush=True)
        ...
        print(f"[Tunnel Note] {err}", flush=True)

@asynccontextmanager
async def lifespan(app: FastAPI):
    # Launch tunnel on server startup
    threading.Thread(target=_start_direct_tunnel, daemon=True).start()
    yield
