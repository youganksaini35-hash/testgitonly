...
async def rename_saved_message_file(
    message_id: int,
    name: str = Form(...),
    ...
    try:
        msg = await client.get_messages('me', ids=message_id)
        if not msg:
            raise HTTPException(status_code=404, detail="File not found in Saved Messages")
        ...
        if folder_id is not None:
            current_meta["folderId"] = folder_id
        elif "folderId" not in current_meta:
            current_meta["folderId"] = "root"
        ...
        return {
            "status": "success",
            "message": f"File #{message_id} successfully renamed to '{new_name}' in Telegram Saved Messages",
            "data": {
                "id": str(message_id),
                "message_id": message_id,
                "name": new_name,
                "size": file_size,
                "folder_id": current_meta.get("folderId", "root"),
                "mimeType": mime_type,
